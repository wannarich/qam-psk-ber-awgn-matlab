function [ber, numBits] = simula_qam2(EbNo, maxNumErrs, maxNumBits)
   ...
   %   8-PSK rectangular.
   %   A diferència d’una 8-PSK clàssica (circular), en aquest cas la constel·lació
   %   té punts distribuïts rectangularment
   
   constelSymb = [1i; (1 + 1i) * 1/sqrt(2); 1; (1 - 1i) * 1/sqrt(2); -1i; (-1 - 1i)* 1/sqrt(2); -1; (-1 + 1i)*1/sqrt(2)];
